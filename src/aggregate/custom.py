    ...
    avg.update(
        {
            "percentage_matches_won": round(
                100 * (avg["total_matches_won"] / avg["total_matches_played"])
            ),
        }
    )
...
def fetch_and_aggregate_custom_team(
    players, db, start=defaultStart, duration=1, end=defaultEnd
):
    s = []
    for i in players:
        s.append("'" + i + "'")
    ...
    playerQuery = (
        """SELECT * FROM playermatches WHERE player in {0} AND year IN {1};""".format(
            q, yearList
        )
    )
